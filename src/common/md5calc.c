...
static void MD5_Round_Calculate(const unsigned char *block, unsigned int *A2,
                                unsigned int *B2, unsigned int *C2, unsigned int *D2) {
	// create X It is since it is required.
	unsigned int X[16]; // 512 bits 64 bytes
	int j, k;
	...
	for (j = 0, k = 0; j < 64; j += 4, k++)
		X[k] = ((unsigned int)block[j])         // 8bytes*4 -> 32bytes conversion
...
//prog
	...
	*D = 0x10325476;
	...
	// Step 3. When bit length cannot be expressed in 32 bytes of low rank, it is a beam raising to a higher rank.
