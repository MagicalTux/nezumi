	...
	if (fp) {
		while(fgets(line, sizeof(line), fp)) { // fgets reads until maximum one less than size and add '\0' -> so, it's not necessary to add -1
			...
			if (inter_guild_fromstr(line, g) == 0 && g->guild_id > 0) {
				...
				numdb_insert(guild_db, (intptr_t)g->guild_id, g);
				guild_check_empty(g);
				guild_calcinfo(g);
				guild_counter++;
			} else {
...
int guild_check_empty(struct guild *g) {
	...
	numdb_erase(guild_db, (intptr_t)g->guild_id);
...
int mapif_parse_GuildInfo(int fd, int guild_id) { // 0x3031 <guild_id>.L - ask for guild
	...
	g = numdb_search(guild_db, (intptr_t)guild_id);
...
int mapif_parse_GuildAlliance(int fd, int guild_id1, int guild_id2, int account_id1, int account_id2, int flag) {
	...
	g[0] = numdb_search(guild_db, (intptr_t)guild_id1);
	g[1] = numdb_search(guild_db, (intptr_t)guild_id2);
...
int mapif_parse_GuildCastleDataLoad(int fd, int castle_id, int idx) { // <Agit>
	...
	{
		case  1:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->guild_id);
		case  2:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->economy);
		case  3:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->defense);
		case  4:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->triggerE);
		case  5:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->triggerD);
		case  6:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->nextTime);
		case  7:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->payTime);
		case  8:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->createTime);
		case  9:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleC);
		case 10:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG0);
		case 11:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG1);
		case 12:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG2);
		case 13:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG3);
		case 14:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG4);
		case 15:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG5);
		case 16:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG6);
		case 17:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->visibleG7);
		case 18:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp0);
		case 19:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp1);
		case 20:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp2);
		case 21:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp3);
		case 22:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp4);
		case 23:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp5);
		case 24:
			return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp6);
		case 25:
...
#ifdef TXT_ONLY
				struct guild *g = numdb_search(guild_db, (intptr_t)gid);
#endif
#ifdef USE_SQL
