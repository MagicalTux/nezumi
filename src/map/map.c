 ...
 *------------------------------------------
 */
TIMER_FUNC(map_clearflooritem_timer) { // if data is set, timer is called before to be executed -> delete it
...
void map_addiddb(struct block_list *bl) {
	...
	numdb_insert(id_db, (intptr_t)bl->id, bl);
}
...
void map_deliddb(struct block_list *bl) {
	...
	numdb_erase(id_db, (intptr_t)bl->id);
}
...
//	skill_clear_unitgroup(&sd->bl);
	...
	numdb_erase(id_db, (intptr_t)sd->bl.id);
  {
	void *p = numdb_search(charid_db, (intptr_t)sd->status.char_id);
	if (p) {
		numdb_erase(charid_db, (intptr_t)sd->status.char_id);
		FREE(p);
	}
  }
//	strdb_erase(nick_db, sd->status.name); // not used
	if (numdb_search(charid_db, (intptr_t)sd->status.char_id) != NULL) /* from freya' forum by LebrEf[TaVu] */
		numdb_erase(charid_db, (intptr_t)sd->status.char_id);

// notify friends
    struct map_session_data * fsd;
	struct friend *frd;
    for(int i = 0; i < sd->friend_num; i++) {
        if ((frd = &sd->friends[i]) && frd->char_id && (fsd = map_charid2sd (frd->char_id)) != NULL) 
            // notify friend
            clif_friend_send_online(fsd->fd, sd->status.account_id, sd->status.char_id, 1);
            // notify player
    }
 ...
 *------------------------------------------
 */
char * map_charid2nick(intptr_t id) {
 ...
 *------------------------------------------
 */
struct block_list * map_id2bl(intptr_t id) {
...
int map_addnpc(int m, struct npc_data *nd) {
	...
	map[m].npc[i] = nd;
	nd->n = i;
	numdb_insert(id_db, (intptr_t)nd->bl.id, nd);
	...
	for(m = 0; m < map_num; m++) {
		for(i = 0; i < map[m].npc_num && i < MAX_NPC_PER_MAP; i++) {
			if (map[m].npc[i] != NULL) {
				...
				numdb_erase(id_db, (intptr_t)map[m].npc[i]->bl.id);
...
void do_init(const int argc, char *argv[]) {
	...
	char *ATCOMMAND_CUSTOM_FILENAME = "conf/atcommand_custom.conf";
	...
	atcommand_custom_read(ATCOMMAND_CUSTOM_FILENAME);
