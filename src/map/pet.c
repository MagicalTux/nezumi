...
static int pet_can_reach(struct pet_data *pd, int x, int y) {
	...
	if (pd->bl.x == x && pd->bl.y == y) // ìØÇ∂É}ÉX
		return 1;
...
int read_petdb() {
	...
	for(i = 0; i < 2; i++) {
		...
		while(fgets(line, sizeof(line), fp)) { // fgets reads until maximum one less than size and add '\0' -> so, it's not necessary to add -1
			...
			//MobID,Name,JName,ItemID,EggID,AcceID,FoodID,"Fullness (1âÒÇÃâaÇ≈ÇÃñûï†ìxëùâ¡ó¶%)","HungryDeray (/min)","R_Hungry (ãÛï†éûâaÇ‚ÇËêeñßìxëùâ¡ó¶%)","R_Full (Ç∆ÇƒÇ‡ñûï†éûâaÇ‚ÇËêeñßìxå∏è≠ó¶%)","Intimate (ïﬂäléûêeñßìx%)","Die (éÄñSéûêeñßìxå∏è≠ó¶%)","Capture (ïﬂäló¶%)",(Name)
