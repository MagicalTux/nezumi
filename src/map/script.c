...
static char pos[11][100] = {"ì™","ëÃ","ç∂éË","âEéË","ÉçÅ[Éu","åC","ÉAÉNÉZÉTÉäÅ[1","ÉAÉNÉZÉTÉäÅ[2","ì™2","ì™3","ëïíÖÇµÇƒÇ¢Ç»Ç¢"};
...
//		}
		p2=(char*)skip_word(p);
		c = *p2; // ñºëOÇadd_strÇ∑ÇÈ
		*p2 = 0; // ñºëOÇadd_strÇ∑ÇÈ
		l = add_str((char*)p);

		parse_cmd = l; // warn_*_mismatch_paramnumÇ
		if (l == search_str("if")) // warn_cmd_no_commaÇ
			parse_cmd_if++;
	...
	for(p++;p && *p && *p!='}';){
		...
		if(*tmpp==':'){
			...
			*skip_word(p)=0;
			...
			strdb_insert(scriptlabel_db,p,(intptr_t)script_pos);	// äOïîóplabel dbìoò^
			*skip_word(p)=c;
			p=tmpp+1;
			continue;
		}
...
int buildin_return(struct script_state *st)
{
	if(st->end>st->start+2){	// ñﬂÇËílóLÇË
...
int buildin_itemskill(struct script_state *st)
{
	...
	// ârè•íÜÇ…ÉXÉLÉãÉAÉCÉeÉÄÇÕégópÇ≈Ç´Ç»Ç¢
	if (sd->skilltimer != -1)
		return 0;
...
//	if(vclass==22) {
//		pc_unequipitem(sd,sd->equip_index[9],0);	// ëïîıäOÇµ
//	}
...
int buildin_waitingroom(struct script_state *st)
{
	...
	if( (st->end > st->start+5) ){
		...
		get_val(st,data);
		if(data->type==C_INT){
			// êVAthenaédól(ãåAthenaédólÇ∆å›ä∑ê´Ç†ÇË)
...
int buildin_globalmes(struct script_state *st)
{
	...
	mes = conv_str(st, &(st->stack->stack_data[st->start + 2])); // ÉÅÉbÉZÅ[ÉWÇÃéÊìæ
	if (mes == NULL)
		return 0;

	if (st->end > st->start + 3){ // NPCñºÇÃéÊìæ(123#456)
...
/*==========================================
 * É`ÉÉÉbÉgÉÅÉìÉoÅ[(ãKíËêlêî)ÉèÅ[Év
 *------------------------------------------
 */
int buildin_warpwaitingpc(struct script_state *st)
{
	...
	for(i=0;i<n;i++){
		struct map_session_data *sd=cd->usersd[0];	// ÉäÉXÉgêÊì™ÇÃPCÇéüÅXÇ…ÅB
...
int buildin_getequipcardcnt(struct script_state *st)
{
	...
	sd=script_rid2sd(st);
	i=pc_checkequip(sd,equip[num-1]);
	if(sd->status.inventory[i].card[0] == 0x00ff){ // êªë¢ïêäÌÇÕÉJÅ[ÉhÇ»Çµ
		push_val(st->stack,C_INT,0);
		return 0;
	}
	do {
...
int buildin_successremovecards(struct script_state *st)
{
	...
	sd=script_rid2sd(st);
	i=pc_checkequip(sd,equip[num-1]);
	if(sd->status.inventory[i].card[0]==0x00ff){ // êªë¢ïêäÌÇÕèàóùÇµÇ»Ç¢
		return 0;
	}
	do{
		if ((sd->status.inventory[i].card[c-1] > 4000 &&
			...
			if ((flag = pc_additem(sd, &item_tmp, 1))) { // éùÇƒÇ»Ç¢Ç»ÇÁÉhÉçÉbÉv
				clif_additem(sd, 0, 0, flag);
	...
	if(cardflag == 1){	// ÉJÅ[ÉhÇéÊÇËèúÇ¢ÇΩÉAÉCÉeÉÄèäìæ
		flag=0;
		...
		item_tmp.card[0]=0,item_tmp.card[1]=0,item_tmp.card[2]=0,item_tmp.card[3]=0;
		pc_delitem(sd,i,1,0);
		if ((flag = pc_additem(sd, &item_tmp, 1))) { // Ç‡ÇƒÇ»Ç¢Ç»ÇÁÉhÉçÉbÉv
			clif_additem(sd, 0, 0, flag);
			map_addflooritem(&item_tmp, 1, sd->bl.m, sd->bl.x, sd->bl.y, NULL, NULL, NULL, sd->bl.id, 0);
		}
		clif_misceffect(&sd->bl, 3);
...
/* ================================================================
 * ÉJÅ[ÉhéÊÇËäOÇµé∏îs slot,type
 * type=0: óºï˚ëπé∏ÅA1:ÉJÅ[Éhëπé∏ÅA2:ïêãÔëπé∏ÅA3:ëπé∏ñ≥Çµ
 * ----------------------------------------------------------------
 */
int buildin_failedremovecards(struct script_state *st)
{
	...
	do {
		if ((sd->status.inventory[i].card[c-1] > 4000 &&
			...
			if(typefail == 2){ // ïêãÔÇÃÇ›ëπé∏Ç»ÇÁÅAÉJÅ[ÉhÇÕéÛÇØéÊÇÁÇπÇÈ
				item_tmp.id=0,item_tmp.nameid=sd->status.inventory[i].card[c-1];
				item_tmp.equip = 0, item_tmp.identify = 1, item_tmp.refine = 0;
				item_tmp.attribute=0;
				item_tmp.card[0]=0,item_tmp.card[1]=0,item_tmp.card[2]=0,item_tmp.card[3]=0;
	...
	if(cardflag == 1){
		...
		if(typefail == 0 || typefail == 2){	// ïêãÔëπé∏
			pc_delitem(sd,i,1,0);
			clif_misceffect(&sd->bl,2);
			return 0;
		}
...
int run_func(struct script_state *st)
{
	...
	if(st->state==RETFUNC){
		...
		i = conv_num(st,& (st->stack->stack_data[st->defsp-4]));				// à¯êîÇÃêîèäìæ
		st->pos=conv_num(st,& (st->stack->stack_data[st->defsp-1]));			// ÉXÉNÉäÉvÉgà íuÇÃïúå≥
		st->script=(char*)conv_num(st,& (st->stack->stack_data[st->defsp-2]));	// ÉXÉNÉäÉvÉgÇïúå≥
		st->defsp=conv_num(st,& (st->stack->stack_data[st->defsp-3]));			// äÓèÄÉXÉ^ÉbÉNÉ|ÉCÉìÉ^Çïúå≥

		pop_stack(st->stack,olddefsp-4-i,olddefsp);		// óvÇÁÇ»Ç≠Ç»Ç¡ÇΩÉXÉ^ÉbÉN(à¯êîÇ∆ïúãAópÉfÅ[É^)çÌèú
