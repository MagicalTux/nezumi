...
unsigned char* parse_script(unsigned char *src, int line)
{
	...
	for(p++;p && *p && *p!='}';){
		...
		if(*tmpp==':'){
			...
			*skip_word(p)=0;
			...
			strdb_insert(scriptlabel_db,p,(intptr_t)script_pos);	// 外部用label db登録
			*skip_word(p)=c;
			p=tmpp+1;
			continue;
		}
	...
	// not need to init new bytes to 0, because here, we always reduce size to adjust at right size
...
int buildin_callfunc(struct script_state *st)
{
	...
	if ((scr = strdb_search(script_get_userfunc_db(), str))) {
		...
		push_val(st->stack,C_INT,(intptr_t)st->script);	// 現在のスクリプトをプッシュ
	...
	if(sd->state.menu_or_input){
		...
		if (name[strlen(name)-1] == '$') { // postfix
			...
			if(st->end>st->start+2){ // 引数1個
				set_reg(sd, num, name, (void*)sd->npc_str); // npc_str = 255 + NULL
			}else{
...
//	if(vclass==22) {
//		pc_unequipitem(sd,sd->equip_index[9],0);	// 装備外し
//	}
