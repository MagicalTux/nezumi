...
int clif_send_sub(struct block_list *bl, va_list ap) {
	...
	case AREA_PET: // to send pet information (hair style) with packet 0x0078 and 0x007b
		if (!(WPACKETW(14) < 24 || WPACKETW(14) > 4000)) {
			if (sd->packet_ver < 12) { // 0: old, 1: 7july04, 2: 13july04, 3: 26july04, 4: 9aug04/16aug04/17aug04, 5: 6sept04, 6: 21sept04, 7: 18oct04, 8: 25oct04/08nov04, 9: 6dec04, 10: 10jan05, 11: 9may05, 12: 28jun05 (by [Yor])
 ...
 *------------------------------------------
*/
int clif_display_globalchat(struct map_session_data *sd, char* message)
{
	int message_len = strlen(message) + 1;

	WPACKETW(0) = 0x8d;
	WPACKETW(2) = message_len + 8;
	WPACKETL(4) = sd->bl.id;
	memcpy(WPACKETP(8), message, message_len);
	clif_send(WPACKETW(2), &sd->bl, AREA_CHAT_WOC);

	WPACKETW(0) = 0x8e;
	WPACKETW(2) = message_len + 4;
	memcpy(WPACKETP(4), message, message_len);
	clif_send(WPACKETW(2), &sd->bl, SELF);
...
static TIMER_FUNC(clif_clearchar_delay_sub) {
...
int clif_clearchar_delay(unsigned int tick, struct block_list *bl, int type) {
	...
	add_timer(tick, clif_clearchar_delay_sub, (intptr_t)tmpbl, type);
...
static int clif_set0078(struct map_session_data *sd) {
	...
	if (sd->disguise > 23 && sd->disguise < 4001) { // mob disguises
		...
		WPACKETB(50) = 5;
		WPACKETB(51) = 0;
		WPACKETW(52) = ((level = status_get_lv(&sd->bl)) > battle_config.max_lv) ? battle_config.max_lv : level;
...
static int clif_set007b(struct map_session_data *sd) {
	...
	if (sd->disguise > 23 && sd->disguise < 4001) { // mob disguises
		...
		WPACKETB(56) = 5;
		WPACKETB(57) = 5;
		WPACKETW(58) = ((level = status_get_lv(&sd->bl)) > battle_config.max_lv) ? battle_config.max_lv : level;
...
int clif_spawnpc(struct map_session_data *sd) {
	...
	clif_send(packet_len_table[0x1d9], &sd->bl, AREA_WOS);
...
//		WPACKETB(12) = 0;
	...
	if (night_flag && !map[sd->bl.m].flag.indoors) { // night_flag: 0=day, 1=night [Yor]
		if (sd->state.night) {
			clif_status_change(&sd->bl, 149, 0);
		}
 ...
 *------------------------------------------
 */
static TIMER_FUNC(clif_waitclose) {
	struct map_session_data *sd;
...
//	struct map_session_data *sd;
	...
	if (bl->type == BL_PC) { // disguises
		...
		if (sd && sd->disguise > 23 && sd->disguise < 4001) {
			clif_send(packet_len_table[0x119], bl, AREA_WOS);
			clif_spawnpc(sd);
		} else
			clif_send(packet_len_table[0x119], bl, AREA);
	} else
		clif_send(packet_len_table[0x119], bl, AREA);
...
//	}
	...
	SENDPACKET(sd->fd, packet_len_table[0x1c9]);
...
{
	if (type == SI_BLANK) // do not send anything if there's no icon
		return 0;
 ...
 *------------------------------------------
 */
int clif_hpmeter(struct map_session_data *sd, struct map_session_data *dstsd) { // NULL: send to any people, other: send to only 1 player
	struct map_session_data *sd2;
	int i, x0, y0, x1, y1;
		...
		for (i = 0; i < fd_max; i++) {
			if (session[i] && (sd2 = session[i]->session_data) && /*sd != sd2 &&*/ sd2->state.auth) {
				if (sd2->bl.m == sd->bl.m &&
				    ...
				    sd2->state.display_player_hp)
				SENDPACKET(i, packet_len_table[0x107] + packet_len_table[0x106]);
			}
		}
	} else {
		if (dstsd->state.auth) {
			if (dstsd->bl.m == sd->bl.m &&
			    dstsd->bl.x >= x0 && dstsd->bl.y >= y0 &&
			    dstsd->bl.x <= x1 && dstsd->bl.y <= y1 &&
			    dstsd->GM_level >= sd->GM_level &&
			    dstsd->state.display_player_hp)
...
//	nullpo_retv(sd); // checked before to call function
	...
	if (g != NULL && strcmp(g->master, sd->status.name) == 0)
		WPACKETL(2) = 0xd7; // guild master
	else
		WPACKETL(2) = 0x57; // other member
...
//	memcpy(WPACKETP(94),"",20); // 本拠地
	...
	if (t == MAX_GUILDCASTLE) strncpy(WPACKETP(94), "Total Domination", 20);
	else if (t ==  1) strncpy(WPACKETP(94), "One Castle", 20);
	else if (t ==  2) strncpy(WPACKETP(94), "Two Castles", 20);
	else if (t ==  3) strncpy(WPACKETP(94), "Three Castles", 20);
	else if (t ==  4) strncpy(WPACKETP(94), "Four Castles", 20);
	else if (t ==  5) strncpy(WPACKETP(94), "Five Castles", 20);
	else if (t ==  6) strncpy(WPACKETP(94), "Six Castles", 20);
	else if (t ==  7) strncpy(WPACKETP(94), "Seven Castles", 20);
	else if (t ==  8) strncpy(WPACKETP(94), "Eight Castles", 20);
	else if (t ==  9) strncpy(WPACKETP(94), "Nine Castles", 20);
	else if (t == 10) strncpy(WPACKETP(94), "Ten Castles", 20);
	else if (t == 11) strncpy(WPACKETP(94), "Eleven Castles", 20);
	else if (t == 12) strncpy(WPACKETP(94), "Twelve Castles", 20);
	else if (t == 13) strncpy(WPACKETP(94), "Thirteen Castles", 20);
	else if (t == 14) strncpy(WPACKETP(94), "Fourteen Castles", 20);
	else if (t == 15) strncpy(WPACKETP(94), "Fifteen Castles", 20);
	else if (t == 16) strncpy(WPACKETP(94), "Sixteen Castles", 20);
	else if (t == 17) strncpy(WPACKETP(94), "Seventeen Castles", 20);
	else if (t == 18) strncpy(WPACKETP(94), "Eighteen Castles", 20);
	else if (t == 19) strncpy(WPACKETP(94), "Nineteen Castles", 20);
	else if (t == 20) strncpy(WPACKETP(94), "Twenty Castles", 20);
	else if (t == 21) strncpy(WPACKETP(94), "Twenty One Castles", 20);
	else if (t == 22) strncpy(WPACKETP(94), "Twenty Two Castles", 20);
	else if (t == 23) strncpy(WPACKETP(94), "Twenty Three Castles", 20);
	else if (t == 24) strncpy(WPACKETP(94), "Twenty Four Castles", 20);
	else strncpy(WPACKETP(94), "None Taken", 20);
		...
		if ((id = g->skill[i].id) > 0) {
			...
			if (g->skill[i].lv < guild_skill_get_max(id)) {
				...
				case GD_BATTLEORDER:
					...
					up = guild_checkskill(g, GD_APPROVAL) > 0 && guild_checkskill(g, GD_EXTENSION) >= 2;
					break;
				case GD_REGENERATION:
					up = guild_checkskill(g, GD_EXTENSION) >= 5 && guild_checkskill(g, GD_BATTLEORDER) > 0;
					break;
				case GD_RESTORE:
					up = guild_checkskill(g, GD_REGENERATION) >= 2;
					break;
				case GD_EMERGENCYCALL:
					up = guild_checkskill(g, GD_GUARDIANRESEARCH) > 0 && guild_checkskill(g, GD_REGENERATION) > 0;
					break;
				case GD_DEVELOPMENT:
					up = 0;
					break;
...
// ------------
TIMER_FUNC(clif_waitauth) { // id = sd->fd, data = sd->bl.id
	struct map_session_data *sd;
...
//	nullpo_retv(sd); // checked before to call function
	...
	//update list of online friends, after sending friend list to char
	struct map_session_data * fsd;
	for(i = 0; i < sd->friend_num; i++) {
        struct friend *frd = &sd->friends[i];
        if (frd->char_id && (fsd = map_charid2sd (frd->char_id)) != NULL) {
			//notify friend
			clif_friend_send_online(fsd->fd, sd->status.account_id, sd->status.char_id, 0);
			//notify player
			clif_friend_send_online(sd->fd, frd->account_id, frd->char_id, 0);
		}
	}
	return;
}
...
void clif_friend_send_online(const int fd, int account_id, int char_id, int flag) {
	...
	WPACKETL( 2) = account_id;
	WPACKETL( 6) = char_id;
	WPACKETB(10) = flag;			//a 1 here means "logged off", go figure...
	SENDPACKET(fd, packet_len_table[0x206]);
...
/*	if (sd) { // checked before
	...
	switch(RFIFOW(fd,0)) { // 0: old, 1: 7july04, 2: 13july04, 3: 26july04, 4: 9aug04/16aug04/17aug04, 5: 6sept04, 6: 21sept04, 7: 18oct04, 8: 25oct04/08nov04, 9: 6dec04, 10: 10jan05, 11: 9may05, 12: 28jun05 (by [Yor])
	case 0x72:
	...
	if ((old_sd = map_id2sd(account_id)) != NULL) {
		...
		sd->fd = fd;
		sd->packet_ver = packet_ver; // 0: old, 1: 7july04, 2: 13july04, 3: 26july04, 4: 9aug04/16aug04/17aug04, 5: 6sept04, 6: 21sept04, 7: 18oct04, 8: 25oct04/08nov04, 9: 6dec04, 10: 10jan05, 11: 9may05, 12: 28jun05 (by [Yor])

		switch(packet_ver) { // 0: old, 1: 7july04, 2: 13july04, 3: 26july04, 4: 9aug04/16aug04/17aug04, 5: 6sept04, 6: 21sept04, 7: 18oct04, 8: 25oct04/08nov04, 9: 6dec04, 10: 10jan05, 11: 9may05, 12: 28jun05 (by [Yor])
		case 12:
...
void antibot_action(struct map_session_data *sd) {
	...
	case 4:
		...
		do {
			if ((rand() % 2) == 0) {
				x = sd->bl.x + 45 + (rand() % AREA_SIZE); // outside area limit (not visible in normal client)
			} else {
				x = sd->bl.x - 45 - (rand() % AREA_SIZE); // outside area limit (not visible in normal client)
			}
			if ((rand() % 2) == 0) {
				y = sd->bl.y + 45 + (rand() % AREA_SIZE); // outside area limit (not visible in normal client)
			} else {
				y = sd->bl.y - 45 - (rand() % AREA_SIZE); // outside area limit (not visible in normal client)
			}
			i++;
...
void clif_parse_LoadEndAck(int fd, struct map_session_data *sd) { // S 0x007d
	...
	if (map[sd->bl.m].flag.gvg) {
		clif_set0199(fd, 3);
	} else if (map[sd->bl.m].flag.guild_dungeon) {
		clif_set0199(fd, 1);
	}
...
//	nullpo_retv(sd); // checked before to call function
	...
	switch (sd->packet_ver) { // 0: old, 1: 7july04, 2: 13july04, 3: 26july04, 4: 9aug04/16aug04/17aug04, 5: 6sept04, 6: 21sept04, 7: 18oct04, 8: 25oct04/08nov04, 9: 6dec04, 10: 10jan05, 11: 9may05, 12: 28jun05 (by [Yor])
	case 12:
		...
		if (sd->client_tick < sd->first_client_tick || gettick_cache < sd->tick_at_start || sd->first_client_tick == 0) {
			...
			if (diff_tick - 5000. > diff_tick2 && (diff_tick - diff_tick2) / diff_tick2 > .1) { // with a marge of 5 sec and more than 10%
				...
				if (!sd->first_check_done || diff_tick2 < 15000.) { // don't display in the fisrt 15 seconds (to parse all cumulativ lags of client). note: tick is sended every 12 sec
					...
					sprintf(message_to_gm, "Possible speed hack usage with player '%s' (account: %d; GM lvl: %d), speed: +%0.02lf%% of normal.", sd->status.name, (int)sd->status.account_id, sd->GM_level, (diff_tick - diff_tick2) / diff_tick2 * 100.);
...
//	nullpo_retv(sd); // checked before to call function
	...
	if ((!pc_isdead(sd) && (sd->opt1 || (sd->opt2 && !(night_flag && (sd->opt2 & STATE_BLIND) == STATE_BLIND)))) || // night_flag: 0=day, 1=night [Yor]
	    sd->skilltimer != -1 ||
...
//		if (ssd == NULL) // not: nullpo_retv(ssd); --> if player is disconnected, it's not an error (not display a message)
//			return;

// removed. a player can ask name before you type @hide
		// first system to detect bot usage
/*		if (ssd != sd && (ssd->status.option & OPTION_HIDE) == OPTION_HIDE && sd->GM_level <= ssd->GM_level) { // not necessary to send message if GM can do nothing
			// we can not ban automatically, because if there is lag, hidden player could be not hidden when other player ask for name.
			char *message_to_gm;
			CALLOC(message_to_gm, char, MAX_MSG_LEN + 45); // max size of msg_txt + char name (24) + account_id (10) + char_id (10) + NULL (1) = max size of msg_txt + 45
			sprintf(message_to_gm, msg_txt(621), sd->status.name, sd->status.account_id, sd->status.char_id); // Possible use of BOT (99% of chance) or modified client by '%s' (account:%d, char_id:%d). This player ask your name when you are hidden.
			clif_wis_message(ssd->fd, wisp_server_name, message_to_gm, strlen(message_to_gm) + 1);
			FREE(message_to_gm);
		}*/
...
//	nullpo_retv(sd); // checked before to call function
	...
	if(sd->npc_id != 0 || sd->opt1 > 0 || sd->status.option & 2 || sd->sc_data[SC_TRICKDEAD].timer != -1 || sd->sc_data[SC_AUTOCOUNTER].timer != -1 || sd->sc_data[SC_BLADESTOP].timer != -1 || (sd->sc_data[SC_GOSPEL].timer != -1 && sd->sc_data[SC_GOSPEL].val4 == BCT_SELF) || sd->sc_data[SC_DANCING].timer != -1)
		return;
	...
	case 0x02: // sitdown
		...
		if (sd->skilltimer != -1) {
			//printf("Sit down when casting a skill (player '%s' (account: %d))!\n", sd->status.name, sd->status.account_id);
...
//	nullpo_retv(sd); // checked before to call function
	...
	// Protection against hacker that dup items:
	// ----------------------------------------
	...
	// to avoid that:
	// when volontary drop an item on the floor (so only here, not in pc_dropitem function) we save player
...
void clif_parse_UseSkillMap(int fd, struct map_session_data *sd)	// S 0x011b <skill ID>.w <map name>.16B
{
	...
	if(sd->npc_id != 0 || sd->vender_id != 0 || sd->sc_data[SC_TRICKDEAD].timer != -1 || sd->sc_data[SC_BERSERK].timer != -1 || (sd->sc_data[SC_GOSPEL].timer != -1 && sd->sc_data[SC_GOSPEL].val4 == BCT_SELF) || sd->sc_data[SC_NOCHAT].timer != -1 || sd->sc_data[SC_WEDDING].timer != -1 || sd->view_class == 22)
		return;
...
//	nullpo_retv(sd); // checked before to call function
	...
	guild_leave(sd, RFIFOL(fd,2), RFIFOL(fd,6), RFIFOL(fd,10), RFIFOP(fd,14)); // S 0159 <guildID>.l <accID>.l <charID>.l <mess>.40B
...
//	nullpo_retv(sd); // checked before to call function
	...
	if (type == 2 && limit == 60 && battle_config.check_ban_bot < 100) {
		if (bl && bl->type == BL_PC && (dstsd = (struct map_session_data *)bl) && tid == bl->id && dstsd->followtimer == -1) { // if not follow someone (@follow GM command)
			char message_to_gm[MAX_MSG_LEN + 100]; // max size of msg_txt + security (char name, char id, etc...) (100)
			sprintf(message_to_gm, "Possible BOT usage with player '%s' (account: %d; GM lvl: %d).", dstsd->status.name, (int)dstsd->status.account_id, sd->GM_level);
	...
	if (!battle_config.auto_muting)
		return;

	if (!battle_config.muting_players) {
		// don't display message if player follows another
		if (!(type == 2 && limit == 60 && bl && bl->type == BL_PC && (dstsd = (struct map_session_data *)bl) && tid == bl->id && dstsd->followtimer != -1))
			clif_displaymessage(fd, msg_txt(511)); // Muting is disabled.
		return;
	}

	if (bl && bl->type == BL_PC && (dstsd = (struct map_session_data *)bl)) {
		if ((tid == bl->id && type == 2 && !sd->GM_level) || (sd->GM_level > dstsd->GM_level)) {
			if (type == 0)
				limit = 0 - limit;
			dstfd = dstsd->fd;
			WPACKETW(0) = 0x14b;
			...
			SENDPACKET(dstfd, packet_len_table[0x14b]);
...
//R 01d1 <Monk id>L <Target monster id>L <Bool>L
	WPACKETW( 0) = 0x1d1;
	WPACKETL( 2) = sd->bl.id;
	WPACKETL( 6) = target->id;
	WPACKETL(10) = type;
	SENDPACKET(sd->fd, packet_len_table[0x1d1]);
...
//	nullpo_retv(sd); // checked before to call function
	...
	WPACKETW(0) = 0x238;
	for(i = 0; i < 10; i++) {
		strncpy(WPACKETP(i * 24 + 2), "Unknown", 24);
		WPACKETL(i * 4 + 242) = 0;
	}
	SENDPACKET(fd, packet_size_table[sd->packet_ver][0x238]);
...
//	nullpo_retv(sd); // checked before to call function
	...
	if (display_unknown_packet) {
		...
		printf("Account ID %d, character ID %d, player name: '%s'.\n", (int)sd->status.account_id, (int)sd->status.char_id, sd->status.name);
	...
	if (sd == NULL) {
		...
		while(RFIFOREST(fd) >= 2 && !session[fd]->eof) {
			...
			if (!sd->state.auth || sd->state.waitingdisconnect) { // 切断待ちの場合パケットを処理しない
				...
				if (clif_parse_func_table[packet_ver][cmd] == clif_parse_WantToConnection) {
					session[fd]->eof = 1;
					return 0;
				} else {
					// パケット処理
					...
					if (clif_parse_func_table[packet_ver][cmd] != clif_parse_TickSend) {
						...
						if (clif_parse_func_table[packet_ver][cmd] != clif_parse_GetCharNameRequest) // not charname solve (mouse can stay in front, and player doesn't ask for)
						...
						if (sd->state.auth) {
							if (sd->status.name != NULL)
								fprintf(fp, "%sPlayer with account ID %d (character ID %d, player name %s) sent wrong packet:" RETCODE,
								        asctime(localtime(&now)), (int)sd->status.account_id, (int)sd->status.char_id, sd->status.name);
							else
