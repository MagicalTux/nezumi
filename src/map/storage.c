...
struct storage *account2storage(intptr_t account_id) {
	...
	if (stor == NULL) {
		...
		numdb_insert(storage_db, (intptr_t)stor->account_id, stor);
	}
...
struct guild_storage *guild2storage(intptr_t guild_id) {
	...
	if (guild_search(guild_id) != NULL) {
		...
		if (gs == NULL) {
			...
			numdb_insert(guild_storage_db, (intptr_t)gs->guild_id, gs);
		}
	}
...
int storage_guild_storageopen(struct map_session_data *sd)
{
	...
	if (sd->status.guild_id <= 0)
		return 2;
	if ((gstor = numdb_search(guild_storage_db, (intptr_t)sd->status.guild_id)) != NULL) {
...
int storage_guild_storage_quit(struct map_session_data *sd)
{
	...
	stor = numdb_search(guild_storage_db, (intptr_t)sd->status.guild_id);
	if (stor) {
		if (stor->modified_storage_flag) {
			intif_send_guild_storage(sd->status.account_id, stor);
			//stor->modified_storage_flag = 0; // because char-server can crash, once modified, guild storage is always saved --> don't change flag
