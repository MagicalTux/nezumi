...
void log_trade(struct map_session_data *sd, struct map_session_data *target_sd)
{
	...
	tmpstr_len += sprintf(tmpstr + tmpstr_len, "* %s [%d(%s:%d)] (ip:%d.%d.%d.%d) %s:" RETCODE, sd->status.name, (int)sd->status.account_id, msg_txt(597), sd->GM_level, sin_addr[0], sin_addr[1], sin_addr[2], sin_addr[3], msg_txt(596)); // lvl // gives
	...
	tmpstr_len += sprintf(tmpstr + tmpstr_len, "* %s [%d(%s:%d)] (ip:%d.%d.%d.%d) %s:" RETCODE, target_sd->status.name, (int)target_sd->status.account_id, msg_txt(597), target_sd->GM_level, sin_addr[0], sin_addr[1], sin_addr[2], sin_addr[3], msg_txt(596)); // lvl // gives
	...
	intif_send_log(2, tmpstr); // 0x3008 <packet_len>.w <log_type>.B <message>.?B (types: 1 GM commands, 2: Trades, 3: Scripts, 4: Vending)
...
//	nullpo_retv(sd); // checked before to call function
	...
	if (((target_sd = map_id2sd(sd->trade_partner)) != NULL) && sd->deal_locked < 1) { // 0: no trade, 1: trade accepted, 2: trade valided (button 'Ok' pressed)
...
//	nullpo_retv(sd); // checked before to call function
	...
	if ((target_sd = map_id2sd(sd->trade_partner)) != NULL) {
		sd->deal_locked = 1; // 0: no trade, 1: trade accepted, 2: trade valided (button 'Ok' pressed)
