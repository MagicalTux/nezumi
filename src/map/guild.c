...
TIMER_FUNC(guild_gvg_eliminate_timer);
...
static int guild_read_castledb(void)
{
	...
	while(fgets(line,sizeof(line),fp)) { // fgets reads until maximum one less than size and add '\0' -> so, it's not necessary to add -1
		...
		numdb_insert(castle_db, (intptr_t)gc->castle_id, gc);
...
struct guild *guild_search(intptr_t guild_id) {
...
struct guild_castle *guild_castle_search(intptr_t gcid) {
...
int guild_payexp_timer_sub(void *key, void *data, va_list ap) {
	...
	nullpo_retr(0, dellist = va_arg(ap, intptr_t *));
...
TIMER_FUNC(guild_payexp_timer) {
	intptr_t dellist[GUILD_PAYEXP_LIST];
	int delp=0,i;
...
void guild_create(struct map_session_data *sd, char *guildname) {
	...
	if (sd->status.guild_id == 0) {
		if (!battle_config.guild_emperium_check || pc_search_inventory(sd, 714) >= 0) {
			...
			strncpy(guild_name, guildname, 24);

			if(check_bad_word(guild_name, strlen(guild_name), sd) == 1)
				return;
...
int guild_created(int account_id,int guild_id)
{
	...
	if (guild_id > 0) {
			...
			sd->guild_sended = 0;
			if ((g = numdb_search(guild_db, (intptr_t)guild_id)) != NULL) {
			...
			if (battle_config.guild_emperium_check)
				pc_delitem(sd, pc_search_inventory(sd,714), 1, 0); // ÉGÉìÉyÉäÉEÉÄè¡ñ’
	} else {
		clif_guild_created(sd, 2); // 0x167 <flag>.b: 0: Guild has been created., 1: You are already in a guild., 2: That Guild Name already exists., 3: You need the necessary item to create a Guild.
	}
...
int guild_npc_request_info(int guild_id,const char *event)
{
	...
	ev->next = (struct eventlist *)numdb_search(guild_infoevent_db, (intptr_t)guild_id);
	numdb_insert(guild_infoevent_db, (intptr_t)guild_id, ev);
...
int guild_recv_info(struct guild *sg) { // 0x3831 <size>.W (<guild_id>.L | <struct guild>.?B) - answer of 0x3031 - if size = 8: guild doesn't exist - otherwise: guild structure
	...
	if ((g = numdb_search(guild_db, (intptr_t)sg->guild_id)) == NULL) {
		CALLOC(g, struct guild, 1);
		numdb_insert(guild_db, (intptr_t)sg->guild_id, g);
		before = *sg;
	...
	if ((ev = numdb_search(guild_infoevent_db, (intptr_t)sg->guild_id)) != NULL) {
		numdb_erase(guild_infoevent_db, (intptr_t)sg->guild_id);
		while(ev) {
...
//	nullpo_retv(sd);
	...
	// inviting to guild is not allowed on woe maps and castles
	if(agit_flag == 1)
	{
		if(map[sd->bl.m].flag.gvg || map[tsd->bl.m].flag.gvg)
		{
...
//	nullpo_retv(sd); // checked before to call function
	...
	if (sd->guild_invite != guild_id) // ä©óUÇ∆ÉMÉãÉhIDÇ™à·Ç§
		return;
...
int guild_member_added(int guild_id, int account_id, int char_id, int flag) { // flag: 0: ok, 1: no guild/guild full
	...
	if (sd == NULL || sd->guild_invite == 0) {
		if (flag == 0) { // flag: 0: ok, 1: no guild/guild full
			...
			intif_guild_leave(guild_id, account_id, char_id, 0, "* Failure of register *");
		}
		return 0;
	}
...
//	nullpo_retv(sd);
	...
	if(agit_flag == 1 && map[sd->bl.m].flag.gvg)
	{
		intif_guild_leave(guild_id, account_id, char_id, 1, mes);	// not sure if 1 is correct flag
		return;
	}
	...
	if (g != NULL) {
		...
		for(i = 0; i < g->max_member; i++)
			...
			if (g->member[i].char_id == char_id) {
				...
				memset(&g->member[i], 0, sizeof(struct guild_member)); // completly erase the member
			}
...
//	if (len < 0 || len > 2000) // unsigned short (can not be negativ)
	if (len > 2000)
		return;
...
int guild_payexp(struct map_session_data *sd, int guild_exp)
{
	...
	if ((c = numdb_search(guild_expcache_db, sd->status.char_id)) == NULL) {
		...
		c->exp = exp2;
		numdb_insert(guild_expcache_db, (intptr_t)c->char_id, c);
	} else {
		c->exp += exp2;
	}
...
//	nullpo_retv(sd); // checked before to call function
	...
	if (flag == 1) {
		...
		if ((g1 = guild_search(sd->status.guild_id)) == NULL ||
		    guild_get_alliance_count(g1, 0) > 3) {
			clif_guild_allianceack(sd, 4);
			clif_guild_allianceack(tsd, 3);
			return;
		}
		if ((g2 = guild_search(tsd->status.guild_id)) == NULL ||
		    guild_get_alliance_count(g2, 0) > 3) {
			clif_guild_allianceack(sd, 3);
		...
		for(i = 0; i < MAX_GUILDALLIANCE; i++) {
			if (g1->alliance[i].guild_id == tsd->status.guild_id &&
			    g1->alliance[i].opposition == 1)
				intif_guild_alliance(sd->status.guild_id, tsd->status.guild_id, sd->status.account_id, tsd->status.account_id, 9);
		}
		for(i = 0; i < MAX_GUILDALLIANCE; i++) {
			if (g2->alliance[i].guild_id == sd->status.guild_id &&
			    g2->alliance[i].opposition == 1)
...
int guild_broken(int guild_id,int flag)
{
	...
	numdb_erase(guild_db,(intptr_t)guild_id);
...
int guild_addcastleinfoevent(int castle_id,int idx,const char *name)
{
	...
	ev->next=numdb_search(guild_castleinfoevent_db,(intptr_t)code);
	numdb_insert(guild_castleinfoevent_db,(intptr_t)code,ev);
...
int guild_castledataloadack(int castle_id,int idx,int value)
{
	...
	if( (ev=numdb_search(guild_castleinfoevent_db,(intptr_t)code))!=NULL ){
		numdb_erase(guild_castleinfoevent_db,(intptr_t)code);
		while(ev) {
			npc_event_do(ev->name);
			ev2 = ev->next;
			FREE(ev);
...
int guild_castlealldataload(int len,struct guild_castle *gc)
{
	...
	for(i = 0; i < n; i++, gc++) {
		...
		memcpy(&c->guild_id, &gc->guild_id, sizeof(struct guild_castle) - ((intptr_t)&c->guild_id - (intptr_t)c));
		if (c->guild_id) {
			if (i != ev)
...
//	nullpo_retr(0, md);
	...
	CALLOC(evname, char, strlen(md->npc_event) + 1);
	...
	add_timer(gettick_cache + battle_config.gvg_eliminate_time, guild_gvg_eliminate_timer, md->bl.m, (intptr_t)evname);

	return;
}

// === COUNT GUILD'S CASTLES ===
// =============================
short guild_checkcastles(struct guild *guild_dat)
{
	struct guild_castle *castle_dat;
	short i, guild_id, castle_counter = 0;

	guild_id = guild_dat->guild_id;

	for(i = 0; i < MAX_GUILDCASTLE; i++)
	{
		castle_dat = guild_castle_search(i);
		if(guild_dat->guild_id == castle_dat->guild_id)
			castle_counter++;
	}
	return castle_counter;
}

// === CHECK IF PLAYER'S GUILD IS ALLIED WITH CASTLE ===
// =====================================================
short guild_isallied(struct guild *guild_dat, struct guild_castle *castle_dat)
{
	int i;

	nullpo_retr(0, guild_dat);

	// castle is not owned by any guild
	if(castle_dat->guild_id == 0)
		return 0;

	// player is on own castle
	if(guild_dat->guild_id == castle_dat->guild_id)
		return 1;

	for(i = 0; i < MAX_GUILDALLIANCE; i++)
	{
		if(guild_dat->alliance[i].guild_id == castle_dat->guild_id)
		{
			if(guild_dat->alliance[i].opposition == 0)
				return 1;
			else
				return 0;
		}
