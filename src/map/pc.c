...
//		printf("Player (id: %d) has quited map-server before to be authentified by char-server.\n", id);
	...
	sd->class_ =  pc_jobid2mapid((unsigned short) sd->status.class);
...
//	nullpo_retr(1, sd); // checked before to call function
	...
	if (n >= 0 && n < MAX_INVENTORY) {
		...
		if (sd->inventory_data[n])
			run_script(sd->inventory_data[n]->script, 0, sd->bl.id, 0);
...
int pc_checkallowskill(struct map_session_data *sd)
{
	...
	if(!(skill_get_weapontype(KN_TWOHANDQUICKEN)&(1<<sd->status.weapon)) && sd->sc_data[SC_TWOHANDQUICKEN].timer!=-1) {
...
TIMER_FUNC(pc_attack_timer) {
	...
	if (sd->opt1 > 0 || sd->status.option & 2 || pc_ischasewalk(sd))
		return 0;
...
int pc_attack(struct map_session_data *sd, int target_id, int type) {
	...
	if (bl == NULL) {
		// if player asks for the fake mob/player (only bot and modified client can see a hidden mob/player)
...
int pc_checkbaselevelup(struct map_session_data *sd)
{
	...
	if (sd->status.base_exp >= next && next > 0) {
		...
		if((sd->status.class&MAPID_UPPERMASK) == MAPID_SUPER_NOVICE) {
	...
	if (map[sd->bl.m].flag.pvp_nightmaredrop) { // Moved this outside so it works when PVP isnt enabled and during pk mode [Ancyker]
		for(j = 0; j < map[sd->bl.m].drop_list_num; j++) { // MAX_DROP_PER_MAP -> now, dynamic
			...
			if (id == -1) {
				...
				for(i = 0; i < MAX_INVENTORY; i++) {
					if (sd->status.inventory[i].nameid == id && rand() % 10000 < per &&
						((type == 1 && !sd->status.inventory[i].equip) || (type == 2 && sd->status.inventory[i].equip) || type == 3)) {
						if (sd->status.inventory[i].equip)
...
//		printf("heal %d %d\n", hp, sp);
	...
	if (sd->sc_count && sd->sc_data[SC_GOSPEL].timer != -1)
		return 0;
...
int pc_jobchange(struct map_session_data *sd, int job, int upper)
{
	...
	sd->class_ = pc_jobid2mapid(sd->status.class);
...
//	nullpo_retv(sd); // checked before to call function
	...
	if ((pos == 0x22)
	    && (id->equip == 2)
	    && (pc_checkskill(sd, AS_LEFT) > 0 || pc_calc_base_job2(sd->status.class) == 12))
	{
		int tpos = 0;
...
{
	if(sd == NULL || dstsd == NULL || sd->status.partner_id > 0 || dstsd->status.partner_id > 0 || sd->class_&JOBL_BABY)
		return -1;
		...
		if ((pl_sd = map_nick2sd(extra_dat[i].name)) != NULL) {
			...
			if (item_id == -1) {
				...
				if (quantity < 0) {
					...
					if (quantity < 0) {
						char output[200];
						sprintf(output, msg_txt(681), -quantity); // Server (special action): you lost %ld zenys.
				...
				// add
					...
					if (quantity > 0) {
						sprintf(output, msg_txt(682), quantity); // Server (special action): you gain %ld zenys.
						clif_displaymessage(pl_sd->fd, output);
						pl_sd->status.zeny += quantity;
						clif_updatestatus(pl_sd, SP_ZENY);
						// line changed -> file must be rewriten
				...
				if (quantity < 0) { // remove items
					for (j = 0; j < MAX_INVENTORY; j++) {
						if (pl_sd->status.inventory[j].nameid == item_id && pl_sd->status.inventory[j].equip == 0) {
							...
							if (quantity < 0) {
								...
								sprintf(output, msg_txt(683), -quantity, item_data->jname); // Server (special action): you lost %ld %s.
								clif_displaymessage(pl_sd->fd, output);
								// line changed -> file must be rewriten
								extra_dat[i].quantity -= quantity;
					...
					for(j = 0; j < loop; j++) {
						...
						if (pc_additem(pl_sd, &item_tmp, quantity) == 0) { // item added
							sprintf(output, msg_txt(684), quantity, item_data->jname); // Server (special action): you obtain %ld %s.
							clif_displaymessage(pl_sd->fd, output);
							// line changed -> file must be rewriten
							extra_dat[i].quantity -= quantity;
							change_flag = 1;
