		...
		if (sd != NULL) {
			if (RFIFOW(fd, 32) == 1) // player not found
				...
				case 6: // change GM level
					switch(RFIFOW(fd, 32)) {
					case 0: // login-server resquest done
						sprintf(output, msg_txt(671), player_name); // Login-server has been asked to change the GM level of the player '%s'.
						break;
					//case 1: // player not found
					case 2: // gm level too low
						sprintf(output, msg_txt(672), player_name); // Your GM level don't authorize you to change the GM level of the player '%s'.
						break;
					case 3: // login-server offline
						sprintf(output, msg_txt(673), player_name); // Login-server is offline. Impossible to change the GM level of the player '%s'.
						break;
					}
					break;
				}
...
int chrif_recv_alone_map_server_flag(int fd) {
	...
	log_refine_level  = RFIFOB(fd,7);
	log_npcsell_level = RFIFOB(fd,8);
	log_pcdrop_level  = RFIFOB(fd,9);
	log_pcpick_level  = RFIFOB(fd,10);
...
int chrif_recv_friends_list(int fd) { // 0x2b25 <size>.W <char_id>.L <friend_num>.W {<friend_account_id>.L <friend_char_id>.L <friend_name>.24B}.x
	...
	sd = map_charid2sd(RFIFOL(fd,4));	//char_id
	if (sd == NULL) 
		return 0;
	...
	// don't send list of friends here. Wait final step of authentification
	// send list to player
	if (friend_num > 0)
...
TIMER_FUNC(check_connect_char_server);
...
//			check_connect_char_server(0, 0, 0, 0); // try immediatly to reconnect if it was the char-server
	...
	while (RFIFOREST(fd) >= 2 && !session[fd]->eof) {
		...
		case 0x2b1d: pc_set_gm_level(RFIFOL(fd,2), (signed char)RFIFOB(fd,6)); break; // 0x2b1d <account_id>.L <GM_Level>.B
		case 0x2b1e: pc_set_gm_level_by_gm(RFIFOL(fd,2), (signed char)RFIFOB(fd,6), RFIFOL(fd,7)); break; // 0x272e/0x2b1e <account_id>.L <GM_level>.B <accound_id_of_GM>.L (GM_level = -1 -> player not found, -2: gm level doesn't authorise you, -3: already right value; account_id_of_GM = -1 -> script)
