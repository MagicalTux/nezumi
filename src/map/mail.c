...
void mail_send(struct map_session_data *sd, char *name, char *message, short flag) {
	...
	if (strcmp(name, "*") == 0) {
		...
		db_sql_escape_string(t_buf, name, strlen(name));
		sql_request("SELECT `account_id`,`name` FROM `%s` WHERE `name` = \"%s\"", char_db, t_buf);
	}

	i = 0;
	db_sql_escape_string(t_name2, sd->status.name, strlen(sd->status.name));
	db_sql_escape_string(t_buf, message, strlen(message));
	while (sql_get_row()) {
		i++;
		if (strcmp(name, "*") == 0) {
			...
			db_sql_escape_string(t_name, sql_get_string(1), strlen(sql_get_string(1)));
