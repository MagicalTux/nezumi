// $Id$
///////////////////////////////////////////////////////////////////////////
// Nezumi login-server remote administration tool
// Ladamin in C by [Yor]
...
//			memcpy(WPACKETP(4), md5bin, 16);
		...
		case 0x7531:	// Displaying of the version of the login-server
			...
			if (((int)RFIFOB(login_fd,5)) == 0) {
				printf("  Nezumi version stable-%d.%d", (int)RFIFOB(login_fd,2), (int)RFIFOB(login_fd,3));
			} else {
				printf("  Nezumi version dev-%d.%d", (int)RFIFOB(login_fd,2), (int)RFIFOB(login_fd,3));
			}
		...
		case 0x7536:	// Displaying of the version of the login-server
			...
			if (((int)RFIFOB(login_fd,6)) == 0)
				...
				printf("-Nezumi mod%d", RFIFOW(login_fd,8));
			else
...
//	set_termfunc(do_final);
	...
	if (defaultlanguage == 'F') {
		printf("Outil d'administration à distance de Nezumi (créé par Yor).\n");
		printf("(pour Nezumi version %d.%d.%d.).\n", ATHENA_MAJOR_VERSION, ATHENA_MINOR_VERSION, ATHENA_REVISION);
	} else {
		printf("Nezumi login-server administration tool (created by Yor).\n");
		printf("(for Nezumi version %d.%d.%d.).\n", ATHENA_MAJOR_VERSION, ATHENA_MINOR_VERSION, ATHENA_REVISION);
	}
