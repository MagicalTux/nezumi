...
//	printf("(" CL_MAGENTA "%d" CL_RESET ") Request save guild - ", g->guild_id);
	...
	db_sql_escape_string(t_name, g->name, strlen(g->name));
	...
	// delete guild if it doesn't have members
	if(guild_member <= 0 && guild_online_member <= 0)
	{
		inter_guild_storage_delete(g->guild_id);
		printf(CL_WHITE "warning: " CL_RESET "guild '%d' doesn't have members. guild removed \n", g->guild_id);
		return -2;
	}
	...
	if (flag & 1 || guild_member == 0) {
		...
		emblem_data[len] = '\0';
		//printf("- emblem_len = %d \n", g->emblem_len);
		db_sql_escape_string(t_master, g->master, strlen(g->master));
		db_sql_escape_string(t_mes1, g->mes1, strlen(g->mes1));
		db_sql_escape_string(t_mes2, g->mes2, strlen(g->mes2));
	...
	if (flag & 2 || guild_member == 0) {
		...
		for(i = 0; i < g->max_member; i++) {
			if (g->member[i].account_id > 0) {
				...
				db_sql_escape_string(t_member, m->name, strlen(m->name));
				sql_request("INSERT INTO `%s` "
	...
	if (flag & 4 || guild_member == 0) {
		...
		for(i = 0; i < MAX_GUILDPOSITION; i++) {
			...
			db_sql_escape_string(t_position, p->name, strlen(p->name));
	...
	if (flag & 8 || guild_member == 0) {
		...
		for(i = 0; i < MAX_GUILDALLIANCE; i++) {
			...
			if (a->guild_id > 0) {
				db_sql_escape_string(t_alliance, a->name, strlen(a->name));
	...
	if (flag & 16 || guild_member == 0) {
		...
		for(i = 0; i < MAX_GUILDEXPLUSION; i++) {
			...
			if (e->account_id > 0) {
				db_sql_escape_string(t_ename, e->name, strlen(e->name));
				db_sql_escape_string(t_emes, e->mes, strlen(e->mes));
...
void inter_guild_init()
{
	...
	{
		printf(CL_WHITE "error: " CL_RESET "failed to reset 'online' in guild database \n");
		exit(0);
	}

	if(!sql_request("UPDATE `%s` SET `connect_member`='0'", guild_db))
	{
		printf(CL_WHITE "error: " CL_RESET "failed to reset 'connect_member' in guild database \n");
		exit(0);
	}
	...
	if(sql_get_row())
	{
		...
		if(guild_counter > 0)
		{
			...
			{
				printf(CL_WHITE "error: " CL_RESET "unable to read max 'guild_id' in guild database \n");
				exit(0);
			}
			if (sql_get_row())
...
//	printf("search_guildname.\n");
	...
	{
		memset(g, 0, sizeof(struct guild));
		db_sql_escape_string(t_name, str, strlen(str));
...
//	inter_guild_tosql(g, 4); // Change guild_position
	db_sql_escape_string(t_position, g->position[idx].name, strlen(g->position[idx].name));
...
int mapif_parse_GuildCastleDataLoad(int fd, int castle_id, int idx) { // <Agit>
	...
	case 18: return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp0);
	...
	case 25: return mapif_guild_castle_dataload(gc->castle_id, idx, gc->Ghp7);
	default:
