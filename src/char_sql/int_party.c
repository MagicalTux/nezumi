...
//	if (p == NULL || party_id == 0 || party_id != p->party_id) {
		...
		// Party exists
			...
			for(i = 0; i < MAX_PARTY; i++) {
				...
				// Check if it's necessary to save
					...
					db_sql_escape_string(t_member, p->member[i].name, strlen(p->member[i].name));
					if (tmp_sql[0] == '\0')
	...
	// new party
		...
		while(i < MAX_PARTY && ((p->member[i].account_id > 0 && p->member[i].leader == 0) || (p->member[i].account_id < 0)))
			i++;
		if (i < MAX_PARTY) {
			db_sql_escape_string(t_name, p->name, strlen(p->name));
	...
	for(i = 0; i < party_num; i++)
		if (partys[i].party_id == party_id) { // if found
			...
			for(j = 0; j < MAX_PARTY; j++)
				partys[i].member[j].online = 0;
			for(k = 0; k < char_num; k++)
				if (char_dat[k].party_id == party_id && online_chars[k] > 0) // just set online visible player
					for(j = 0; j < MAX_PARTY; j++)
						if (partys[i].member[j].account_id == char_dat[k].account_id && strcmp(partys[i].member[j].name, char_dat[k].name) == 0) {
							partys[i].member[j].online = 1;
							break;
						}
